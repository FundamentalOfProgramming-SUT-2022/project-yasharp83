...
void compare(){
    ...
    if (File_exist(input_file[0])||File_exist(input_file[1])){
        error_file() ; 
        return ;
    }
...
void output_to_board(){
    str_to_file(BOARD_FILE , output->str) ; 
}
//////main
void pain(char * command){
        cnt_arman = 0 ; 
        ...
        for (int i = 0 ; i < cnt_arman ; i++){
            ...
            if (!strcmp(input_kind , "exit")){
            ...
            else if (!strcmp(input_kind , "save")){
                clear_input() ; 
                if (!strcmp(CUR_FILE , BOARD_FILE)){
                    clear() ; 
                    show_boarder() ; 
                    refresh() ; 
                    cur_x = 1 ; 
                    ...
                    input_file[0][pos] = '\0' ; 
                    for (int i = 0 ; i < MX_CMD ; i++)
                        CUR_FILE[i] = '\0' ; 
                    strcpy(CUR_FILE , input_file[0]) ; 
                    create_file() ; 
                    struct DY_str * DA = init_DY_str() ; 
                    file_to_str(BOARD_FILE , DA) ; 
                    str_to_file(CUR_FILE , DA->str) ; 
                }
                else {
                    struct DY_str * DA = init_DY_str() ; 
                    file_to_str(BOARD_FILE , DA) ; 
            ...
            else if (!strcmp(input_kind , "saveas")){
                clear_input() ; 
                clear() ; 
                show_boarder() ; 
                refresh() ; 
                ...
                struct DY_str * DA = init_DY_str() ; 
                file_to_str(BOARD_FILE , DA) ; 
                str_to_file(input_file[0] , DA->str) ; 
            }
            else if (!strcmp(input_kind , "open")){
                if (!strcmp(CUR_FILE , BOARD_FILE)){
                    clear() ; 
                    show_boarder() ; 
                    refresh() ; 
                    ...
                    cur_y = MX_Y - 1 ; 
                    move(cur_y , cur_x) ; 
                    printw("%s" , "enter the name(saving last file): ") ; 
                    cur_x+=36 ; 
                    move(cur_y , cur_x) ; 
                    char c ; 
                    int pos = 0 ; 
                    ...
                    for (int i = 0 ; i < MX_CMD ; i++)
                        CUR_FILE[i] = '\0' ; 
                    strcpy(CUR_FILE , input_file[0]) ; 
                    file_to_str(CUR_FILE , DA) ; 
                    str_to_file(BOARD_FILE , DA->str) ; 
                }
                else {
                    cur_x = 1 ; 
                    cur_y = MX_Y - 1 ; 
...
void visual(){
    ...
    while(1){
        ...
        if (c=='!'){
            ...
            int line = y + l_up ;
            int xx = x - 1 ; 
            int posf = find_pos_by_line(linef , xxf , BOARD_FILE) ; 
            int pos = find_pos_by_line(line , xx , BOARD_FILE) ; 
            int dis = pos - posf ; 
            char A[2000] ; 
            if (dis > 0)
                sprintf(A , "copystr --file %s --pos %d:%d -size %d -f\n" , BOARD_FILE , linef , xxf , dis) ; 
            else
                sprintf(A , "copystr --file %s --pos %d:%d -size %d -b\n" , BOARD_FILE , linef , xxf , dis) ; 
            pain(A) ; 
            VIM_MODE = 2 ; 
            normal() ; 
        }
            ...
            pain(A) ; 
            if (dis > 0)
                sprintf(A , "removestr --file %s --pos %d:%d -size %d -f\n" , BOARD_FILE , linef , xxf , dis) ; 
            else
                sprintf(A , "removestr --file %s --pos %d:%d -size %d -b\n" , BOARD_FILE , linef , xxf , dis) ; 
            pain(A) ; 
            VIM_MODE = 2 ; 
            normal() ; 
        }
...
void normal(){
    ...
    while(1){
        ...
        if (c=='l'){
            ...
            int xx = x - 1; 
            char A[MX_CMD] ; 
            if (clip_board->str==NULL)
                continue ; 
            printw("%s" , clip_board->str) ; 
            sprintf(A , "insertstr --file %s --str %s --pos %d:%d\n" , BOARD_FILE , clip_board->str , line , xx) ; 
            pain(A) ; 
        }
    }
...
void insert(){
    ...
    while(1){
        ...
        if (c=='!'){
            ...
            sprintf(B , "insertstr --file %s --str %s --pos %d:%d\n" , BOARD_FILE , A , line , xx) ; 
            pain(B) ; 
        }
    }
}
...
void show_boarder(){
    FILE * my_file = fopen(BOARD_FILE , "r") ; 
    char A[MX_CMD] ; 
    int l = 1 ; 
    while(fgets(A , MX_CMD , my_file)!=NULL){
        if (l < l_up){
            l++ ; 
            continue ; 
        }
    ...
    printw("\n") ; 
    while(l <= MX_Y-3 + l_up - 1){
        printw("~\n") ; 
        l++ ; 
    }
    attron(COLOR_PAIR(1)) ; 
    printw("%s | " , STR_VIM_MODE[VIM_MODE] ) ; 
    if (strcmp(CUR_FILE , BOARD_FILE)==0)
        printw(" %s\n" , "empty!") ; 
    else    
        printw(" %s\n" , CUR_FILE) ; 
...
int main(){
    for (int i = 0 ; i < MX_CMD ; i++)
        CUR_FILE[i] = '\0' ; 
    strcpy(CUR_FILE , BOARD_FILE) ; 
    FILE * my_file = fopen(BOARD_FILE , "w") ; 
    fclose(my_file) ; 
    initscr() ; 
    start_color() ; 
    keypad(stdscr , true) ; 
